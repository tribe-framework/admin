...
<?php } ?>
    ...
    <form id="dtList" action="/admin/delete-dt-rows" method="post">
        ...
        <table class="my-4 list table table-sm table-striped table-hover datatable border-bottom border-dark <?=($listed_fields_number>=6 ? 'cols-6-or-more' : '')?>" data-jsonpath="list-json" data-type="<?=$type?>" data-role="<?=$_GET['role'] ?? ''?>" data-lazyload=<?=($ids_number>5000 || ($types[$_type]['list_lazyload'] ?? false)) ? "true" : "false"?> >
