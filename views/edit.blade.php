    ...
    @else
        ...
        @if ((isset($_GET['id']) && $post['type'] == $type) || !isset($_GET['id']))
            ...
            <form method="post" class="edit_form" action="/admin/json" autocomplete="off">
                ...
                {{-- handling of type and type change --}}
                ...
                @if ($types['webapp']['display_activity_log'])
                    <div class="form-group">
                        ...
                        <div class="collapse border border-light overflow-auto" id="mysql_log">
                            <div class="container">
                                @if (count($post['mysql_activity_log'] ?? []))
                                    @php
                                    ...
                                    @foreach ($access_log as $key => $log)
                                        <p class="mb-0 small px-2 row {{ $key%2 ? 'bg-light' : 'bg-white' }}">
                                            <span class="text-muted mr-2 col-1 border-right border-black-50 text-center">
                                                {{ (int) $key + 1 }}
                                            </span>
                                            <span class="text-warning text-center fw-bold col-2 border-right">
                                                {{ $log['time'] }}
                                            </span>
                                            <span class="col">
                                                user <a href="/admin/edit?type=user&id={{$log['user_id']}}" class="text-secondary" target="_blank">{{ $log['user_name'] ? "{$log['user_id']} ({$log['user_name']})" : "{$log['user_id']}" }}</a> {{ $log['message'] }}
                                            </span>
                                        </p>
                                    @endforeach
                                @else
